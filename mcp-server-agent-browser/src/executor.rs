...
pub async fn exec_browser(args: Vec<String>, timeout_secs: Option<u64>) -> Result<String, String> {
    exec_browser_with_stdin(args, timeout_secs, None).await
}

/// Execute an agent-browser CLI command with optional stdin and return stdout.
pub async fn exec_browser_with_stdin(
    args: Vec<String>,
    ...
    if let Some(input) = stdin {
        ...
        if let Some(mut child_stdin) = child.stdin.take() {
            child_stdin
                ...
                .await
                .map_err(|e| format!("Failed to write to agent-browser stdin: {e}"))?;
            drop(child_stdin);
        }
    }
